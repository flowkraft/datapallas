...
export default function InvoicesPage() {
  ...
  useEffect(() => { fetchInvoices(page, search) }, [page, fetchInvoices])
  useEffect(() => { setPage(1); fetchInvoices(1, search) }, [search, fetchInvoices])
  ...
  return (
    <div className="space-y-4">
      ...
      <div id="invoices-header" className="flex items-center justify-between">
        <h1 id="invoices-page-title" className="text-xl font-semibold text-base-content">Invoices</h1>
        <Link href="/admin/invoices/new">
          <Button id="btn-new-invoice" size="sm">
      ...
      {/* Table */}
      <div id="invoices-table-card" className="rounded border border-base-300 bg-base-100 overflow-hidden">
        <Table id="invoices-table">
          <TableHeader>
            <TableRow className="bg-base-200 border-b border-base-300">
              <TableHead className="text-xs uppercase tracking-wide text-base-content/60">Invoice</TableHead>
              <TableHead className="text-xs uppercase tracking-wide text-base-content/60">Customer</TableHead>
              <TableHead className="text-xs uppercase tracking-wide text-base-content/60">Issued</TableHead>
              <TableHead className="text-xs uppercase tracking-wide text-base-content/60">Due</TableHead>
              <TableHead className="text-right text-xs uppercase tracking-wide text-base-content/60">Amount</TableHead>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow>
                <TableCell colSpan={7} className="text-center py-6 text-sm text-base-content/60">No invoices found.</TableCell>
              </TableRow>
            ) : (
              invoices.map((invoice) => (
                <TableRow key={invoice.id} className="border-b border-base-200 hover:bg-base-200/50">
                  <TableCell className="text-sm font-medium text-base-content">{invoice.invoiceNumber}</TableCell>
                  <TableCell>
                    <div className="text-sm text-base-content">{invoice.customerName}</div>
                    <div className="text-xs text-base-content/40">{invoice.customerEmail || invoice.customerId}</div>
                  </TableCell>
                  <TableCell className="text-sm text-base-content/60">{formatDate(invoice.issueDate)}</TableCell>
                  <TableCell className="text-sm text-base-content/60">{formatDate(invoice.dueDate)}</TableCell>
                  <TableCell className="text-right text-sm font-medium text-base-content">
                    {formatCurrency(invoice.totalAmount, invoice.currency)}
                  </TableCell>
                  <TableCell>
                    <span className={getStatusStyle(invoice.status)}>{invoice.status}</span>
                  </TableCell>
                  <TableCell>
