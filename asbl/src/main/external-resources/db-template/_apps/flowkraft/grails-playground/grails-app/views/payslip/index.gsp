...
<body>
    ...
    <div class="space-y-4">
        ...
        <div class="flex justify-between items-center mb-3" id="payslips-header">
            <h1 class="h5 font-semibold text-base-content mb-0" id="payslips-page-title">Payslips</h1>
            <a href="${createLink(action: 'create')}" class="btn btn-sm btn-neutral" id="btn-new-payslip">
        ...
        <div class="mb-3" id="payslips-search">
            <g:form action="index" method="GET" class="inline-block">
                <input type="text" class="input input-sm w-full" name="search" id="payslips-search-input"
                       value="${params.search}" placeholder="Search..." style="max-width: 200px;"/>
            </g:form>
        </div>

        <!-- Table - matches Next.js structure -->
        <div class="card bg-base-100 border border-base-300" id="payslips-table-card">
            <div class="overflow-x-auto">
                <table class="table" id="payslips-table">
                    <thead>
                        <tr>
                            <th class="text-sm text-base-content/60 uppercase">Payslip</th>
                            <th class="text-sm text-base-content/60 uppercase">Employee</th>
                            <th class="text-sm text-base-content/60 uppercase">Dept</th>
                            <th class="text-sm text-base-content/60 uppercase">Period</th>
                            <th class="text-sm text-base-content/60 uppercase text-right">Net</th>
                    ...
                    <tbody>
                        <g:if test="${payslipList}">
                            <g:each in="${payslipList}" var="payslip" status="i">
                                <tr class="payslip-row" id="payslip-row-${payslip.id}">
                                    <td class="text-sm font-medium">${payslip.payslipNumber}</td>
                                    <td>
                                        <div class="text-sm">${payslip.employeeName}</div>
                                        <div class="text-sm text-base-content/60">${payslip.employeeId}</div>
                                    </td>
                                    <td class="text-sm text-base-content/60">${payslip.department ?: '-'}</td>
                                    <td class="text-sm text-base-content/60">
                                        <g:formatDate date="${payslip.payPeriodStart}" format="MMM d"/> -
                                        <g:formatDate date="${payslip.payPeriodEnd}" format="MMM d"/>
                                    </td>
                                    <td class="text-sm font-medium text-right">${payslip.formatAmount(payslip.netAmount)}</td>
                                    <td>
                                        <span class="badge ${payslip.status in ['viewed', 'downloaded'] ? 'badge-success' : 'badge'} text-sm">
                                            ${payslip.status}
                                        </span>
                                    </td>
                                    <td>
                        ...
                        <g:else>
                            <tr>
                                <td colspan="7" class="text-center py-4 text-base-content/60 text-sm">
                                    No payslips found.
                                </td>
                            </tr>
                        </g:else>
