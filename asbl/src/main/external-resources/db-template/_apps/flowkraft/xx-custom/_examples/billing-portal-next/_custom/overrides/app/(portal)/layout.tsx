...
export default async function PortalLayout({ children }: { children: React.ReactNode }) {
  ...
  // x-pathname is set by middleware.ts; the Grails twin reads actionName in layouts/portal.gsp.
  const publicPay = (await headers()).get("x-pathname")?.startsWith("/portal/pay") ?? false;
        ...
        <nav className="navbar w-full py-0 px-4">
          <div className="flex-1 flex items-center gap-4">
            <Link href="/portal" className="flex items-center gap-2 shrink-0 no-underline">
              ...
              <span className="text-xl font-bold tracking-tight">Northwind Traders</span>
            </Link>
            {s && !publicPay ? (
          ...
          <div className="flex items-center gap-1">
            {!publicPay ? (
              <>
                {s?.role === "ADMIN" ? (
                  <Link href="/admin" className="btn btn-ghost btn-sm normal-case">Admin</Link>
                ) : null}
                {s ? (
                  <>
                  ...
                  <Link href="/login" className="btn btn-ghost btn-sm normal-case">Sign in</Link>
                )}
              </>
            ) : null}
