...
/**
 * Delete confirmation — the 1:1 mirror of the Grails _confirmDeleteCustomer.gsp modal (same ids,
 * same copy).
 *
 * Deleting a customer also deletes their login, so it must never happen on a single stray click.
 * The dialog only ASKS: a customer who still has invoices is refused by deleteCustomer in
 * lib/actions.ts even after a Yes, because their invoices must be dealt with first.
 *
 ...
 * so getElementById would always return the FIRST row's copy and the wrong customer would be
 * deleted. That is also why this is split in two: DeleteCustomerButton sits in each row,
 * DeleteCustomerModal is rendered exactly once by the page.
 */
export function DeleteCustomerButton({
  customerId,
  customerEmail,
  customerName,
  // The list renders a compact row action; the customer detail page a full-size outlined button.
  ...
  const onClick = () => {
    ...
    what.textContent = customerName;        // …retarget the single modal at THIS row
    idInput.value = String(customerId);
    modal.showModal();
  };
  // The id is keyed on the email, not the name: it is the customer's unique column, and the Grails
  // twin keys the same button the same way.
  return (
    <button type="button" id={`btn-delete-${customerEmail}`} className={className} onClick={onClick}>Delete</button>
  );
}

/** Rendered ONCE per page. `action` is the deleteCustomer server action, passed down from the page. */
export function DeleteCustomerModal({ action }: { action: (formData: FormData) => void | Promise<void> }) {
