...
>
  <div class="space-y-4" style="margin: 35px;">
    @if (isJasperFileInfo(modalConfigurationTemplateInfo.fileInfo)) {
    <div class="alert alert-info" style="margin-bottom: 15px;">
      ...
      <strong>JasperReport@if (modalConfigurationTemplateInfo.fileInfo.type == 'config-jasper-legacy-reports') { legacy}</strong> — This report is defined by a <code>.jrxml</code> template. Configuration is read-only. To modify the report, edit the <code>.jrxml</code> file in Jaspersoft Studio@if (modalConfigurationTemplateInfo.fileInfo.type == 'config-jasper-legacy-reports') { 6.x, which writes the format this report uses}.
    </div>
    }
    ...
    <div style="display:grid;grid-template-columns:repeat(12,1fr);gap:1rem">
      ...
      <div style="grid-column:span 10">
        <input
          ...
          autofocus
          required
          [readonly]="isJasperFileInfo(modalConfigurationTemplateInfo.fileInfo)"
        />
      </div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(12,1fr);gap:1rem">
      ...
      <div style="grid-column:span 10">
        <input
          ...
          [disabled]="isJasperFileInfo(modalConfigurationTemplateInfo.fileInfo)"
        />
        <label
        ...
        &nbsp;&nbsp;
        @if (modalConfigurationTemplateInfo.fileInfo.type == 'config-reports' || isJasperFileInfo(modalConfigurationTemplateInfo.fileInfo)) {
  ...
  <div ngProjectAs="[footer]">
    ...
    <button
      ...
      [disabled]="
        ...
        isJasperFileInfo(modalConfigurationTemplateInfo.fileInfo) ||
        !isModalDirty()
      "
    >
