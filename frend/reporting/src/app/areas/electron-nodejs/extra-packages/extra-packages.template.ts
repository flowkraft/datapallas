  ...
  @if (stateStore.configSys.sysInfo.setup.chocolatey.isChocoOk) {
    <div>
      ...
      @for (extraPackage of extraPackages; track $index) {
        <div
          id="extraPackageCard-{{extraPackage.id}}"
          [ngClass]="{'card border-primary': extraPackage.status != 'not-installed', 'card card-border' : extraPackage.status == 'not-installed' }"
        >
          <div class="card-title">
            <h3 id="extraPackageHeader-{{extraPackage.id}}">
              <a href="{{extraPackage.website}}" target="_blank">{{extraPackage.website}}</a
              >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
              @if (extraPackage.status == 'not-installed') {
                <em id="extraPackageStatus-{{extraPackage.id}}">{{ 'AREAS.INSTALL-SETUP-UPGRADE.COMPONENTS.EXTRA-PACKAGES.INSTALLED.NOT' | translate }}</em>
                &nbsp;&nbsp;
                <button
                  id="btnInstallPackage-{{extraPackage.id}}"
                  type="button"
                  class="btn btn-outline btn-primary btn-xs align-middle"
                  [disabled]="extraPackage.busy"
                  (click)="installPackage(extraPackage)"
                >
                  @if (extraPackage.busy) {
                    <span class="loading loading-spinner loading-sm"></span>
                  }
                  {{ 'AREAS.INSTALL-SETUP-UPGRADE.COMPONENTS.EXTRA-PACKAGES.ACTIONS.INSTALL-NOW' | translate }}
                </button>
              }
              @if (extraPackage.status != 'not-installed') {
                <strong id="extraPackageStatus-{{extraPackage.id}}">{{ 'AREAS.INSTALL-SETUP-UPGRADE.COMPONENTS.EXTRA-PACKAGES.INSTALLED.ALREADY' | translate }}</strong>
                ...
                &nbsp;&nbsp;
                <button
                  id="btnUninstallPackage-{{extraPackage.id}}"
                  type="button"
                  class="btn btn-outline btn-error btn-xs align-middle"
                  [disabled]="extraPackage.busy"
                  (click)="unInstallPackage(extraPackage)"
                >
                  @if (extraPackage.busy) {
                    <span class="loading loading-spinner loading-sm"></span>
                  }
                  {{ 'AREAS.INSTALL-SETUP-UPGRADE.COMPONENTS.EXTRA-PACKAGES.ACTIONS.UNINSTALL-NOW' | translate }}
                </button>
              }
            </h3>
          </div>
