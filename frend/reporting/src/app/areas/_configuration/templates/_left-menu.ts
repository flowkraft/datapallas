...
<ul class="sidebar-menu menu w-full py-2">
  <li class="header menu-title">
    ...
    @if ((settingsService.currentConfigurationTemplate?.type === 'config-jasper-reports' || settingsService.currentConfigurationTemplate?.type === 'config-jasper-legacy-reports')) {
      <br />
  ...
  <li>
    <a id="leftMenuGeneralSettings" href="#" [routerLink]="[
      ...
      ]" skipLocationChange="true" routerLinkActive="menu-active">
  ...
  @if (xmlSettings?.documentburster?.settings?.capabilities?.reportgenerationmailmerge && settingsService.currentConfigurationTemplate?.type !== 'config-jasper-reports' && settingsService.currentConfigurationTemplate?.type !== 'config-jasper-legacy-reports') {
  <li>
